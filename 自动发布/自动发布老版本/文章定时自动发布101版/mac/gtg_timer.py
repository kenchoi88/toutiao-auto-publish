        ...
        for sub_idx in range(1, sub_rounds + 1):
            ...
            for name in list(accounts):
                ...
                if not ok:
                    ...
                    stage2_consecutive_fail += 1
                    if stage2_consecutive_fail >= STAGE2_MAX_FAIL:
                        log(chr(10) + f"!! [Stage2] 连续 {stage2_consecutive_fail} 次失败触发熔断（最近: {reason}），脚本停止")
                        try:
                            subprocess.run([
                                "osascript", "-e",
                                f'display notification "Stage 2 连续{stage2_consecutive_fail}次失败已停止，请检查罐头状态" with title "文章定时 Stage2 熔断" sound name "Sosumi"'
                            ], capture_output=True)
                        except Exception:
                            pass
                        return {
