...
ACCOUNT_CLASS  = "account-cBAwuL"  # [v1102.9 mac 2026-05-16] 罐头当前真 hash, CDP probe 实证 120 真号 0 伪号; 罐头升级人工改 1 行
WAIT_LOAD      = 4
...
def _read_whitelist_with_quota():
    """读白名单 A 列。[v1102.9 缺哥拍 2026-05-16 一步到位 fix] 忽略 B 列,统一 quota=9999
    Why: catchup 写「漏数」/ B 列残值都会让 acc_count<quota_map 对冲跳过接续点
         统一强写 9999 (5/15 故障 3 取消 cap 决议,自然停于素材池),acc_count<9999 永远 True
    业务效果: 中断重启无需重跑 catchup,白名单残值不影响,接续点账号必 eligible
    素材池上限管控:每号实际发文量受素材池总量限制,不再人工 cap
    """
    if not os.path.exists(CONFIG_EXCEL):
        return []
    try:
        ...
        for row in ws_r.iter_rows(min_row=2, max_col=1, values_only=True):
            ...
            result.append((name, 9999))  # [v1102.9 缺哥拍 2026-05-16] 取消 cap, 自然停于素材池
        wb.close()
        return result
    except Exception:
        return []
        ...
        for channel in ["系统通知", "审核通知"]:
            ...
            (function(){{
                ...
                for(var i=0; i<items.length; i++){{
                    ...
                    // [v1102.9-mac-fix] indexOf===0 太严,unread badge 数字会顶到 0 位;放宽 includes
...
def run_death_grip(
    ...
    doc_pool,
    main_ws,
    sub_rounds=1,              # [v1102.9 缺哥拍 2026-05-16] 默认 1, 调用方传动态 _quota_per
    max_fail_per_sub=3,
    ...
    def _do_publish(name, doc, round_label, is_retry=False):
        ...
        if name not in notice_checked_set:
            _sys_n, _audit_n = check_system_notice(ws_url, name)  # [v1102.9] 双频道独立计数
...
def main():
    ...
    # [v1102] 读「本轮已发」sheet → {账号: 已发次数}
    # [v1102.9 缺哥拍 2026-05-16 接续 fix] 删除 sent_accounts_set 预注入 — 它本是「本小轮已发」语义,
    # 预注入让 V1102.6 接续算法重排首位被 _is_eligible 跳过,实际首篇 publish 落到顺位下一个 eligible 号
    # acc_count<quota 检查独立挡超发,sent_accounts_set 保留小轮内防重复发即可
    ...
    # 读取账号配置.xlsx - 白名单sheet (A 列账号 + B 列 quota,catchup.py 写入)
    quota_map = {}
    try:
            ...
            for wn, _wq in _wl_q:
                for a in _orig_accounts:
                    if a not in _seen and (wn in a or a in wn):
                        ...
                        _seen.add(a)
                        break
            # [v1102.9 缺哥拍 2026-05-16] 白名单只优先排序,不剔除账号 — 不在白名单的账号追加末尾保证全员都发
            # Why: catchup 写白名单时基于 quota_target=5 剔除 sent>=5 账号; 新 quota 算法用全员 120 算可能=8,
            #      原剔除的账号 (8-sent) 篇没发漏掉, 现追加到末尾让全员一起补到 quota
            for a in _orig_accounts:
                if a not in _seen:
                    _new_accounts.append(a)
                    _seen.add(a)
            accounts = _new_accounts
            log(f"账号配置.xlsx[白名单]已加载，白名单 {len(_wl_map)} 个优先排序，全员 {len(accounts)} 个账号(首位={accounts[0] if accounts else '空'})")
    except Exception as _e:
        log(f"读取账号配置.xlsx[白名单]失败: {_e}")

    # [V1102.6 缺哥拍 2026-05-09] 接续算法移出 if _wl_q,无论是否有白名单都从 last_published 接续
    ...
    # [v1102.9 缺哥拍 2026-05-16] 统一 quota 算法 — 每号分够 quota 篇, 缺 N 补 N, 一篇不少
    # 分母用「全员账号 _full_accounts_count」(白名单只缩本次发文范围, 不影响 quota 算分母)
    # 例: 960 总文稿 / 120 全员 = quota=8 余 0;1000/120 = quota=8 余 40 留池
    # 已发够 quota 的账号 acc_count>=quota 自动停, 没发够的继续发到 quota
    if _full_accounts_count > 0:
        _quota_per = max(1, (len(docs) + sent_total) // _full_accounts_count)
        _quota_余 = (len(docs) + sent_total) % _full_accounts_count
    else:
        _quota_per = 1; _quota_余 = 0
    quota_map = {a: _quota_per for a in accounts}
    log(f"本次发布: {len(accounts)} 个账号(白名单/全员={_full_accounts_count})，{len(docs)} 篇文档, 每号 quota={_quota_per} 篇 (素材{len(docs)}+已发{sent_total}=总{len(docs)+sent_total}/{_full_accounts_count} 全员={_quota_per} 余 {_quota_余} 篇留素材池, 无 cap, 缺 N 补 N)")

    # [V1102.5] 中断恢复算法 — 从 Win 标答 line 2434-2440 移植,4 mac 微头条 V1102 落地以来一直缺这段
    # 业务效果:中断重启时按 sent_total 推算"第 X 小轮第 Y 账号断点" + 起跑位置
    if sent_count_map and len(accounts) > 0:
        q_est = (len(docs) + sent_total) // len(accounts)
        log(f"素材池剩 {len(docs)} 篇 + 已发累计 {sent_total} 篇 = 总 {len(docs) + sent_total} 篇 / {len(accounts)} 账号 = 每号 quota={q_est}")
        ...
        log(f"    本小轮({current_round})剩 {remaining_in_round} 账号 + 后续 {q_est - current_round} 小轮各 {len(accounts)} 账号")

    doc_pool = list(docs)

    # 调用死磕主循环
    # [v1102.9 缺哥拍 2026-05-16] sub_rounds 动态对齐每号 quota, 不再写死 5(老版 cap 5 + 阿良 5→10 修法都退役)
    result = run_death_grip(
        accounts=accounts,
        per_account_quota=quota_map,
        doc_pool=doc_pool,
        main_ws=main_ws,
        sub_rounds=_quota_per,
        max_fail_per_sub=3,
