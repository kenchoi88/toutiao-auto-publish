...
ACCOUNT_CLASS  = "account-"  # 前缀模糊抗罐头 hash 升级;wrapper 误命中靠 selector 里 text<40 过滤排除

# 发文 webview zoom 90% 的特殊账号(罐头给的 dpr=1.125 而非 1.25),坐标算屏幕时打 0.9 折
ZOOM_FIX_ACCOUNTS = {"一笑酒暖花深"}
WAIT_LOAD      = 4
...
def _init_run_dir():
    global LOG_FILE, FAIL_FILE, NOTICE_FILE, NOTICE_CHECKED_FILE, LAST_PUBLISHED_FILE, ALERT_FILE, VIOLATION_FILE, AUDIT_FILE, RUN_REPORT_DIR
    ...
    AUDIT_FILE     = os.path.join(RUN_REPORT_DIR, "审核通知.txt")  # [v1102.9]
...
# 5 类硬终止 reason — 命中即永久放弃,不再尝试
HARD_TERMINATE_REASONS = {"封号", "禁言", "失登", "信用分过低", "侧边栏未找到"}  # [V1102.5] 失登加回(铁律 line 678)
...
def _append_hard_terminate(name, reason, count_so_far):
    """命中 5 类硬终止时,写入"硬终止账号"sheet。永久放弃,不再尝试。"""
    try:
        _ensure_config_excel()
        wb = openpyxl.load_workbook(CONFIG_EXCEL)
        if "硬终止账号" not in wb.sheetnames:
...
def collect_accounts(main_ws):
    ...
    # [v1102.9 缺哥拍 2026-05-16 B 方案] 主循环 same_count 容错避免 loading 误判到底, sleep 0.3→0.5
    same_count = 0
    for _ in range(1000):
        v = js(main_ws, f"""
        (function(){{
            var items = [...document.querySelectorAll('[class^="{ACCOUNT_CLASS}"]')].filter(e=>{{var t=(e.textContent||'').trim();return t.length>0&&t.length<40;}});
            var names = [];
            for(var i=0;i<items.length;i++){{
                var t = items[i].textContent.trim();
                if(t) names.push(t);
    ...
    # [v1102.9 缺哥拍 2026-05-16 B 方案] 兜底循环:滚到底 + 等 2s + 扫, 直到连续 3 次没新增(最多 20 轮)
    no_new_count = 0
    bottom_round = 0
        ...
        if v:
            ...
            for n in names:
                if n not in seen:
                    ...
                    added += 1
        if added:
            log(f"  兜底第 {bottom_round} 轮:滚到底补收 {added} 个账号(累计 {len(accounts)})")
            no_new_count = 0
        else:
            no_new_count += 1
...
def check_system_notice(ws_url, account_name):
    """
    [v1102.9] 导航到消息中心 → 点击 系统通知 + 审核通知 频道
    → 读取 2 天内(今天+昨天)的完整消息原文
    → 系统通知右侧 → 系统通知.txt(NOTICE_FILE);审核通知右侧 → 审核通知.txt(AUDIT_FILE)独立两文件
    数据源: 右侧主消息容器 .chat-container-list (非 body.innerText 全页,免左侧列表预览污染)
    """
    try:
        today = datetime.now()
        ...
        wsc = ws_connect(ws_url, timeout=8)
        time.sleep(2.5)
        channel_notices = {"系统通知": [], "审核通知": []}  # [v1102.9] 双频道独立
        ...
        for channel in ["系统通知", "审核通知"]:
            ...
            time.sleep(1.5)
            # [v1102.9] ReactVirtualized 默认视口顶部 mount,最新消息在底部 unmount;先滚到底让最新进 DOM
            js(wsc, "(function(){var b=document.querySelector('.chat-container-list');if(b){b.scrollTop=b.scrollHeight;}return 'ok';})()", 303)
            time.sleep(1.5)

            # [v1102.9] 改用 .chat-row 节点遍历 + textContent,绕开 innerText 视口裁切
            # (chat-container-list 是 overflow:auto 滚动容器,所有 20+ row 全在 DOM,但 innerText 只返视口可见,导致最新消息在底部被裁)
            result = js(wsc, f"""
            (function() {{
                ...
                var rows = document.querySelectorAll('.chat-container-list .chat-row');
                function dateInfo(line) {{
                    ...
                    // [v1102.9] 相对时间格式 — 头条对 24 小时内的消息显示成"刚刚/N 分钟前/N 小时前",全在 2 天窗口内
                    if (/^刚刚$/.test(line)) return true;
                    if (/^\\d+\\s*分钟前$/.test(line)) return true;
                    if (/^\\d+\\s*小时前$/.test(line)) return true;
                    return null;
                }}
                var results = [];
                var currentDate = '';
                ...
                for (var i = 0; i < rows.length; i++) {{
                    ...
                    if (!t) continue;
                    if (r.classList.contains('time-stamp')) {{
                        currentDate = t;
                        inWindow = (dateInfo(t) === true);
                    }} else if (inWindow) {{
            ...
            if result:
                try:
                    ...
                    channel_notices[channel].extend(msgs)  # [v1102.9] 各归各的
                except:
                    pass

        wsc.close()

        # [v1102.9] 双 TXT 独立写入,砍违规提醒整段
        ts_str = datetime.now().strftime("%Y-%m-%d %H:%M")
        counts = {"系统通知": 0, "审核通知": 0}
        for channel, msgs in channel_notices.items():
            if not msgs:
                continue
            target = NOTICE_FILE if channel == "系统通知" else AUDIT_FILE
            content_str = f"\n[{ts_str}] 账号 {account_name} 2 天内 {channel} ({len(msgs)} 条):\n"
            for msg in msgs:
                content_str += f"\n--- 通知 ---\n{msg}\n"
            content_str += "\n" + "=" * 60 + "\n"
            with open(target, "a", encoding="utf-8") as f:
                f.write(content_str)
            counts[channel] = len(msgs)

        sys_n = counts["系统通知"]
        audit_n = counts["审核通知"]
        if sys_n == 0 and audit_n == 0:
            log("  系统/审核通知: 2 天内无新通知")
        else:
            log(f"  ⚠ 系统通知 {sys_n} 条 → 系统通知.txt / 审核通知 {audit_n} 条 → 审核通知.txt")
        return sys_n, audit_n
    except Exception as e:
        log(f"  系统通知检测出错: {e}")
        return 0, 0
...
def ensure_zuopin_shengming_checked(ws):
    """[V1102.7 左右拍 2026-05-11] 确保「作品声明:取材网络」已勾选。
    成功: 已勾 / click 后校验 OK → True
    失败: 找不到 / click 失败 / 回读不一致 → False (该篇硬保护跳过,不进 HARD_TERMINATE)
    业务原因: 平台 AI 生成内容未标注 → 失实表述扣 10 分,比不发出去更糟。"""
    try:
        ret = js(ws, """
        (function(){
          ...
          for (var i = 0; i < ALL.length; i++) {
            var t = (ALL[i].textContent || '').trim();
            if (t.indexOf('取材网络') >= 0 && t.length < 20) {
              var cb = ALL[i].closest('label')?.querySelector('input[type="checkbox"]')
                    || ALL[i].previousElementSibling
                    || ALL[i].parentElement?.querySelector('input[type="checkbox"]');
        ...
        """)
        if ret == 'already_checked':
            log(f"  作品声明:取材网络 → 已勾选(已校验)")
            return True
        if ret != 'clicked':
            log(f"  ★ 作品声明:取材网络 找不到元素 ({ret}) — 该篇硬保护跳过")
            return False
        time.sleep(0.4)
        verify = js(ws, """
        (function(){
          ...
          for (var i = 0; i < ALL.length; i++) {
            ...
            if (t.indexOf('取材网络') >= 0 && t.length < 20) {
              var cb = ALL[i].closest('label')?.querySelector('input[type="checkbox"]')
                    || ALL[i].previousElementSibling
                    || ALL[i].parentElement?.querySelector('input[type="checkbox"]');
              if (cb) return cb.checked ? 'verified_ok' : 'verify_failed';
            }
          }
          return 'verify_no_element';
        })()
        """)
        if verify == 'verified_ok':
            log(f"  作品声明:取材网络 → click 后已校验勾选 ✓")
            return True
        log(f"  ★ 作品声明:取材网络 click 后校验失败 ({verify}) — 该篇硬保护跳过")
        return False
    except Exception as e:
        log(f"  ★ 作品声明勾选异常: {e} — 该篇硬保护跳过")
        return False
...
def publish_article(ws_url, doc_path, main_ws, name="", _credit_out=None):
    ...
    # 导航后 target id 会变，从 tabs 里重新找 weitoutiao/publish 页面
    new_ws_url = None
    for _ in range(12):
        tabs = get_tabs()
        for t in tabs:
            if "weitoutiao/publish" in t.get("url","") and "webSocketDebuggerUrl" in t:
    ...
    # 点底部"文档导入"按钮（JS 直接 click，不依赖 pyautogui）
    # 取 webview 真实屏幕坐标（从主窗口查，和Mac版逻辑一致）
    wv_s = js(main_ws, """
    (function(){
    ...
    # 用真实鼠标点击文档导入按钮（JS注入无法触发，和Mac版cliclick等效）
    ...
    log(f"  文档导入按钮屏幕坐标: ({import_x},{import_y}) [zoom_ratio={zoom_ratio:.3f} main_dpr={_main_dpr:.3f} pub_dpr={_pub_dpr:.3f}]")
    ...
    editor_x = wv0['sx'] + 400
    editor_y = wv0['sy'] + 200
    win32api.SetCursorPos((editor_x, editor_y))
    time.sleep(0.2)
    ...
    # 从主窗口重新取webview坐标，算选择文档按钮的真实屏幕坐标
    wv_s2 = js(main_ws, """
    (function(){
        var wvs = document.querySelectorAll('webview');
        var maxArea = 0, best = null;
    ...
    """, 66)
    wv1 = json.loads(wv_s2) if wv_s2 else wv0
    screen_x = wv1['sx'] + int(sb['bx'] * zoom_ratio)
    screen_y = wv1['sy'] + int(sb['by'] * zoom_ratio)
    log(f"  webview原点:({wv1['sx']},{wv1['sy']}) 按钮CSS:({sb['bx']},{sb['by']}) => 选择文档屏幕:({screen_x},{screen_y}) [zoom_ratio={zoom_ratio}]")
    ...
    # Ctrl+A → Ctrl+V → Enter（只发一次，Windows文件对话框一次即可）
    ...
    if credit_score is not None:
        if credit_score < 80:
            log(f"  信用分: {credit_score} ★★★ 危险")
        elif credit_score < 90:
            log(f"  信用分: {credit_score} ★ 警告")
        else:
            log(f"  信用分: {credit_score}")
    else:
        log("  信用分: 未读取到")
    ...
    # [V1102.7 左右拍 2026-05-11] 作品声明:取材网络 — 失败硬保护跳过该篇
    if not ensure_zuopin_shengming_checked(wsc):
        wsc.close()
        return False, "作品声明勾选失败"
...
def run_death_grip(
    ...
    sub_rounds=1,              # [v1102.9 缺哥拍 2026-05-16] 默认 1, 调用方传动态 default_q
    ...
    """死磕循环:大循环 N 小轮 + 外层无限磕。
    - 5 类硬终止(失登/封号/禁言/低信用/侧边栏未找到) → 即刻永久放弃,写"硬终止账号"sheet
    ...
    if NOTICE_CHECKED_FILE and os.path.exists(NOTICE_CHECKED_FILE):
        try:
            ...
            if notice_checked_set:
                log(f"从 notice_checked.txt 恢复 {len(notice_checked_set)} 个已检查账号(中断恢复)")
        except Exception as _e:
            log(f"notice_checked.txt 读取失败: {_e}")
    ...
    def _do_publish(name, doc, round_label, is_retry=False):
        ...
        # [v1102/v1102.9] 每账号当天只读 1 次审核/系统通知 — 持久化,中断恢复后仍跳过
        if name not in notice_checked_set:
            _sys_n, _audit_n = check_system_notice(ws_url, name)
            total_notices += (_sys_n + _audit_n)
            ...
            try:
                ...
                log(f"写 notice_checked.txt 失败: {_e}")
            time.sleep(2)
        else:
        ...
        try:
            ...
            if success:
                ...
                try:
                    ...
                    log(f"写 last_published.txt 失败: {_e}")
    ...
    def _handle_failure(name, reason, sub_fail_count, sub_skipped):
        """处理 _do_publish 返回的失败。命中 5 类硬终止即永久放弃。"""
        if reason in HARD_TERMINATE_REASONS:
            ...
            log(f"  ★ 5 类硬终止: {name} -> {reason} (本次已发 {cnt_so_far} 篇,永久放弃)")
            return True
...
def main():
    ...
    # [v1102.9 缺哥拍 2026-05-16] 保留全员账号数用于 quota 算法 — 白名单只缩范围, quota 算分母用全员
    _full_accounts_count = len(accounts)

    # per_account_quota: 本次每个账号要发几篇
    per_account_quota = {}

    # [v1102] quota 动态算 = (素材池 + 已发累计) // 账号数 — 用户拿走/添加素材都自动反映
    ...
    if whitelist_with_q:
        ...
        # [v1102.9 缺哥拍 2026-05-16] 白名单只优先排序,不剔除账号 — 不在白名单的追加末尾保证全员都发, B 列 quota 忽略
        ...
        log(f"账号配置.xlsx[白名单]已加载，白名单 {len(wl_map)} 个优先排序，全员 {len(accounts)} 个账号(首位={accounts[0] if accounts else '空'})")
    # [v1102.9 缺哥拍 2026-05-16] 统一 quota 算法 — 分母用 _full_accounts_count(全员), 无 cap, 缺 N 补 N
    default_q = max(1, (len(docs) + sent_total) // _full_accounts_count) if _full_accounts_count else 1
    _quota_余 = (len(docs) + sent_total) % _full_accounts_count if _full_accounts_count else 0
    for a in accounts:
        per_account_quota[a] = default_q
    total_target = sum(per_account_quota.values())
    log(f"本次发布: {len(accounts)} 个账号(白名单/全员={_full_accounts_count})，{len(docs)} 篇文档, 每号 quota={default_q} 篇 (素材{len(docs)}+已发{sent_total}=总{len(docs)+sent_total}/{_full_accounts_count} 全员={default_q} 余 {_quota_余} 篇留素材池, 无 cap, 缺 N 补 N)")
    if sent_count_map:
        log(f"素材池剩 {len(docs)} 篇 + 已发累计 {sent_total} 篇 = 总 {len(docs) + sent_total} 篇 / {len(accounts)} 账号 = 每号 quota={default_q}")
        # 算断点
        current_round = sent_total // len(accounts) + 1
        done_in_round = sent_total % len(accounts)
        remaining_in_round = len(accounts) - done_in_round
        log(f"中断恢复:第 {current_round} 小轮第 {done_in_round + 1} 账号断点(已发 {sent_total} 篇)")
        log(f"    本小轮({current_round})剩 {remaining_in_round} 账号 + 后续 {default_q - current_round} 小轮各 {len(accounts)} 账号")

    # [V1102.6 左右拍 2026-05-10] 接续算法移出 if whitelist_with_q,无论是否有白名单都从 last_published 接续
    ...
    if LAST_PUBLISHED_FILE and os.path.exists(LAST_PUBLISHED_FILE):
        try:
            ...
            log(f"last_published.txt 读取失败: {_e}")
    if _last_published_acc and accounts:
        ...
        if _last_idx >= 0:
            ...
            log(f"中断处自动接续:最近 publish「{_last_published_acc}」(idx={_last_idx}) → 从下一位「{accounts[0]}」起跑")
    ...
    # ========== 收尾:硬终止账号汇报 ==========
    if dead_terminated:
        log(f"\n★ 5 类硬终止账号 {len(dead_terminated)} 个 (已写入'硬终止账号'sheet,需人工处理):")
    ...
    summary = [
        ...
        f"硬终止号: {len(dead_terminated)} 个 (失登/封号/禁言/低信用/找不到)",
