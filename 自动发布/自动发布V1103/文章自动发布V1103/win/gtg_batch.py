...
NOTICE_PROBE_ENABLED = False  # [V1102.4] 文章件不探测通知 (微头条 NOTICE 抓全足够, 文章件去掉省 5-8s/篇 + 减焦点抖)
ALERT_FILE     = None
VIOLATION_FILE = None
...
def publish_article(ws_url, doc_path, main_ws, name="", _credit_out=None):
    ...
    log(f"  选择文档屏幕坐标: ({screen_x},{screen_y}) [zoom_ratio={zoom_ratio}]")
...
def run_death_grip(
    ...
    def _do_publish(name, doc, round_label, is_retry=False):
        ...
        # [V1102.4] 文章件 NOTICE_PROBE_ENABLED=False 跳过探测 (微头条已抓, 文章件复抓无价值)
        if NOTICE_PROBE_ENABLED and name not in notice_checked_set:
...
def main():
    ...
    # [V1102.5] 中断恢复算法 — 4 件文章件 V1102.4 落地以来一直缺这段
