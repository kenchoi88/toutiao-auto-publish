...
NOTICE_PROBE_ENABLED = False  # [V1102.4] 文章件不探测通知 (微头条 NOTICE 抓全足够)
ALERT_FILE       = None
VIOLATION_FILE   = None
...
def run_death_grip(
    ...
    def _do_publish(name, doc, round_label, is_retry=False):
        ...
        # [V1102.4] NOTICE_PROBE_ENABLED=False 跳过
...
def main():
    ...
    try:
        ...
        # 预注入让 V1102.6 接续算法重排首位被 _is_eligible 跳过, acc_count<quota 检查独立挡超发
        if sent_count_map:
    ...
    # [V1102.6 缺哥拍 2026-05-09] 接续算法移出 if _wl_map,无论是否有白名单都从 last_published 接续
    ...
    # [v1102.9 缺哥拍 2026-05-16] 统一 quota 算法 — 动态 (素材+已发)//全员账号 整除, 余数留素材池, 无 cap
    # 分母用 _full_accounts_count(全员), 白名单只优先排序不影响 quota 算法
