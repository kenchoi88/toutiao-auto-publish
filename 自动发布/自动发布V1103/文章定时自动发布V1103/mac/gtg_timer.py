...
自动排程规则：
  - 早窗 07:01 起：发文数≥1 的账号各发 1 篇
  - 中窗 11:01 起：发文数≥2 的账号各发 1 篇
  - 晚窗 15:01 起：发文数=3 的账号各发 1 篇
...
def _expand_tasks(accounts_quota, date_str):
    ...
    """
    GAP_MIN = 1
    # [缺哥 2026-05-11 拍] 缺 N 篇 → 排前 N 个窗(剩 1=早 / 剩 2=早+中 / 剩 3=早+中+晚)
    # q=3 没发过 → 早+中+晚;q=2 → 早+中;q=1 → 早 (晚窗保留给 q=3 的账号)
    windows = [
...
def _check_notice_once(ws_url, account_name):
    ...
    # [V1102.4] NOTICE_PROBE_ENABLED=False 直接返回
    if not NOTICE_PROBE_ENABLED:
        return
    ...
    try:
        ...
        if pending_rows:
            ...
            for r in pending_rows:
                log(f"  - {r[0]} 漏 {r[1]} 篇")
            log(f"本次未齐活,「本轮已发」 sheet 保留(下次启动接着跑)")
        else:
...
# 漏发账号在 Stage 1 跑完后,这里把它们在"now + 30min"重新排程,直到全部发完或命中 5 类硬终止
# 5 类硬终止 = {失登, 封号, 禁言, 信用分过低, 侧边栏未找到}
...
def run_death_grip_timer(
    ...
    doc_pool,
    main_ws,
    sub_rounds=1,              # [v1102.9 缺哥拍 2026-05-16] 默认 1, 调用方传动态 quota_total
    max_fail_per_sub=3,
    initial_dead=None,
):
    """
    Stage 2: 用 publish_article_timer 立即模式 (timer_time=None) 补尾漏发账号——直接发，不再走定时排程。
    - 5 类硬终止 → 立即写硬终止 sheet 永久放弃
    - 其他失败 → 本小轮 max_fail_per_sub 次失败 → 跳本小轮(下小轮恢复)
    - 大循环 N 小轮 + 外层无限磕,直到全发完 / 5 类硬终止 / Ctrl+C
    """
...
def main():
    ...
    # [v1102.9 缺哥拍 2026-05-16] 总是先 collect 全员, 白名单只优先排序不剔除, B 列 quota 忽略, 跟微头条新版对齐
    ...
    if _last_published_acc and all_accounts:
        ...
        if _last_idx >= 0:
            ...
            log(f"中断处自动接续:最近 publish「{_last_published_acc}」(idx={_last_idx}) → 从下一位「{all_accounts[0]}」起跑")

    # 准备文档池(提前到 quota 算之前)
    doc_pool = list(get_docs())
    docs_count = len(doc_pool)

    # 读「本轮已发」 → 算 quota_total = (素材+已发)//账号 整除, 余数留素材池, 无 cap, 缺 N 补 N
    # [v1102.9 缺哥拍 2026-05-16] 取消 cap 3 — 自然停于素材池, 跟微头条新版对齐
    sent_count_map = _read_sent_with_count()
    sent_total = sum(sent_count_map.values())
    if len(all_accounts) > 0:
        quota_total = max(1, (docs_count + sent_total) // len(all_accounts))
        _quota_余 = (docs_count + sent_total) % len(all_accounts)
    else:
        quota_total = 1; _quota_余 = 0
    log(f"本次发布: {len(all_accounts)} 个账号, {docs_count} 篇文档, 每号 quota={quota_total} 篇 (素材{docs_count}+已发{sent_total}=总{docs_count+sent_total}/{len(all_accounts)}={quota_total} 余 {_quota_余} 篇留素材池, 无 cap, 缺 N 补 N)")
    ...
    # 5 类硬终止集合 + 硬终止账号字典(本次累计) + 写"硬终止账号"sheet
    HARD_TERMINATE_REASONS = {"封号", "禁言", "失登", "信用分过低", "侧边栏未找到"}  # [V1102.5] 失登加回(铁律 line 678)
    ...
    def _terminate(name, reason, doc_path):
        ...
        dead_terminated[name] = (reason, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), cnt)
        _append_hard_terminate(name, reason, cnt)
        log(f"  ★ 5 类硬终止: {name} -> {reason} (本次已发 {cnt} 篇,永久放弃)")
    ...
    for idx, (name, timer_time) in enumerate(tasks):
        ...
        if not pos:
            ...
            _terminate(name, "侧边栏未找到", doc_path)  # 5 类硬终止,不 requeue
    ...
    if remaining_accounts and doc_pool:
        ...
        log(f"# 漏发账号立即发布（不走定时），死磕直到全部成功 / 5 类硬终止 / Ctrl+C")
        log(f"# 直到全部排程成功 / 命中 5 类硬终止 / 你 Ctrl+C")
        log(f"{'#'*60}")
        try:
