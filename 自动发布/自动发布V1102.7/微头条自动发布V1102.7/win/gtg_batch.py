...
def publish_article(ws_url, doc_path, main_ws, name="", _credit_out=None):
    ...
    log(f"  文档导入按钮屏幕坐标: ({import_x},{import_y})")
    ...
    log(f"  webview原点:({wv1['sx']},{wv1['sy']}) 按钮CSS:({sb['bx']},{sb['by']}) => 选择文档屏幕:({screen_x},{screen_y})")
...
def run_death_grip(
    ...
    sub_rounds=5,              # 1 大循环 = N 小轮(微头条 5,文章/定时 3)
...
def main():
    ...
    if whitelist_with_q:
        ...
        default_q = max(1, min(5, (len(docs) + sent_total) // len(accounts))) if accounts else 1  # [V1102.6 左右拍 2026-05-10] cap 5
        for a in accounts:
            matched_q = None
