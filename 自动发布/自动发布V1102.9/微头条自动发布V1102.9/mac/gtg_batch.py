...
ACCOUNT_CLASS  = "account-"  # [v1102.8 移植] 罐头 v1.7.11+ className hash 升级,改前缀模糊匹配抗以后变化
WAIT_LOAD      = 4
...
def _read_whitelist_with_quota():
    """读白名单 A 列。[v1102.9] 取消 xlsx 配额读取,每号硬 5 篇"""
    if not os.path.exists(CONFIG_EXCEL):
        return []
    try:
        ...
        for row in ws_r.iter_rows(min_row=2, max_col=1, values_only=True):
            ...
            result.append((name, 9999))  # [v1102.9] 取消 quota 上限,自然停于素材池
        wb.close()
        return result
    except Exception:
        return []
...
def collect_accounts(main_ws):
    ...
    for _ in range(1000):
        v = js(main_ws, f"""
        (function(){{
            var items = document.querySelectorAll('[class^="{ACCOUNT_CLASS}"]');
            var names = [];
            for(var i=0;i<items.length;i++){{
                var t = items[i].textContent.trim();
                t = t.replace(/^\d+\s*/, "");  // [v1102.9] strip 前导未读角标 e.g. "2 一乐也" → "一乐也" 防 dedupe 漏
                if(t) names.push(t);
            }}
...
def main():
    ...
    try:
        ...
        if _wl_q:
            ...
            for a in accounts:
                matched_q = None
                for wn, wq in _wl_map.items():
                    if wn in a or a in wn:
                        matched_q = wq
                        break
                if matched_q is not None:
                    quota_map[a] = matched_q
            if quota_map:
                log(f"按独立 quota 跑, 配额合计 {sum(quota_map.values())} 篇")
    except Exception as _e:
    ...
    if sent_count_map and len(accounts) > 0:
        ...
        log(f"  [v1102] 素材池剩 {len(docs)} 篇 + 已发累计 {sent_total} 篇 = 总 {len(docs) + sent_total} 篇 / {len(accounts)} 账号 = 每号 quota={q_est}")
