...
def run_death_grip(
    ...
    sub_rounds=10,              # 1 大循环 = N 小轮(微头条 10,文章/定时 3)
...
def main():
    ...
    if whitelist_with_q:
        ...
        default_q = max(1, (len(docs) + sent_total) // len(accounts)) if accounts else 1  # [2026-05-12] 去除硬 cap, 配额按文档池均分
        for a in accounts:
            matched_q = None
    ...
    if sent_count_map:
        log(f"  [v1102] 素材池剩 {len(docs)} 篇 + 已发累计 {sent_total} 篇 = 总 {len(docs) + sent_total} 篇 / {len(accounts)} 账号 = 每号 quota={default_q}")
        # 算断点
        ...
        log(f"  [v1102] 中断恢复:第 {current_round} 小轮第 {done_in_round + 1} 账号断点(已发 {sent_total} 篇)")
    ...
    if _last_published_acc and accounts:
        ...
        if _last_idx >= 0:
            ...
            log(f"  [v1102.6] 中断处自动接续:最近 publish「{_last_published_acc}」(idx={_last_idx}) → 从下一位「{accounts[0]}」起跑")
