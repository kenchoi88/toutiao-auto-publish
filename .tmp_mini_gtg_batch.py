...
def main():
    ...
    # 缺哥 2026-05-01 临时补漏: quota=3, 从今日 log 反推每号已发数, quota[a] = 3 - 已发(缺 N 补 N,不多发不少发)
    QUOTA_TARGET = 3
    _hist = {}
    try:
        import re as _re_q
        _task_pat = _re_q.compile(r"\[剩余\s*\d+\s*篇\]\s*\[\S+?\]\s*(?:\[补发\]\s*)?(\S+?)\s*->")
        _ok_pat = _re_q.compile(r"OK 发布成功")
        _cur_acc = None
        if LOG_FILE and os.path.exists(LOG_FILE):
            with open(LOG_FILE, encoding="utf-8") as _hf:
                for _ln in _hf:
                    _m = _task_pat.search(_ln)
                    if _m:
                        _cur_acc = _m.group(1).strip()
                        continue
                    if _ok_pat.search(_ln) and _cur_acc:
                        _hist[_cur_acc] = _hist.get(_cur_acc, 0) + 1
                        _cur_acc = None
        if _hist:
            log(f"[补漏模式] 今日 log 解析: {len(_hist)} 号已发, 累计 {sum(_hist.values())} 篇")
    except Exception as _e_h:
        log(f"[补漏模式] log 解析失败,fallback 平均配额: {_e_h}")
        _hist = {}

    if _hist:
        quota_map = {a: max(0, QUOTA_TARGET - _hist.get(a, 0)) for a in accounts}
        _total_remaining = sum(quota_map.values())
        _zero_quota = sum(1 for v in quota_map.values() if v == 0)
        log(f"[补漏模式] quota={QUOTA_TARGET},总待补 {_total_remaining} 篇 (已满 quota 跳过 {_zero_quota} 号)")
    else:
        # fallback 老逻辑(平均配额)
        quota = len(docs) // len(accounts) if len(accounts) > 0 else 1
        quota = max(quota, 1)
        quota_map = {a: quota for a in accounts}
        log(f"fallback 平均配额: 每账号 {quota} 篇")
