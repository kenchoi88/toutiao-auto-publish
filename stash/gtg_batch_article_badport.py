...
def _read_whitelist_with_quota():
    """读取白名单 sheet：A列账号名，B列配额（空=1）。返回 {账号: 配额}"""
    if not os.path.exists(CONFIG_EXCEL):
        return {}
    try:
        wb = openpyxl.load_workbook(CONFIG_EXCEL, read_only=True, data_only=True)
...
def scroll_find_account(main_ws, name):
    ...
    # 滚动找不到，尝试搜索框输入账号名过滤（Win版：PowerShell剪贴板 + 模拟键盘）
    log(f"  滚动未找到账号，尝试搜索框输入: {name}")
    search_info = js(main_ws, """
    (function(){
        ...
        return JSON.stringify({
            ...
            sx: Math.round(window.screenX + r.left + r.width/2),
            sy: Math.round(window.screenY + r.top + r.height/2)
        });
    })()
    """, 14)
    if not search_info:
        return None
    si = json.loads(search_info)

    import win32gui, win32con, win32api, psutil
    # 罐头窗口置顶
    ...
    if electron_pid:
        ...
        def _enum_top(h, _):
            ...
            _u32.GetWindowThreadProcessId(h, ctypes.byref(pid_buf))
    ...
    # 真实鼠标点击搜索框
    win32api.SetCursorPos((si['sx'], si['sy']))
    time.sleep(0.2)
    ...
    # PowerShell 写剪贴板 + Ctrl+V 粘贴账号名
    safe_name = name.replace("'", "''")
    subprocess.run(['powershell', '-Command', f"Set-Clipboard -Value '{safe_name}'"], capture_output=True)
    time.sleep(0.3)
...
def check_system_notice(ws_url, account_name):
    ...
    → 读取今天和昨天的消息原文写入 NOTICE_FILE
    → 命中违规关键词的额外写入 VIOLATION_FILE
...
def publish_article(ws_url, doc_path, main_ws, account_name="", _credit_out=None):
    ...
    """, 63)
    if wv_s_after:
        wv0 = json.loads(wv_s_after)
        import_x = wv0['sx'] + pi['x']
        import_y = wv0['sy'] + pi['y']
        log(f"  激活后重新校正坐标: ({import_x},{import_y})")

    # 等"已恢复上次编辑"提示条消失（"撤销"按钮），最多等8秒
    for _ in range(16):
        has_dismiss = js(wsc, """
        (function(){
    ...
    time.sleep(0.05)
    win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)
    log(f"  真实鼠标点击文档导入 ({import_x},{import_y})")
    time.sleep(1.5)
    ...
    # 根据信用分设置头条首发复选框（LABEL.byte-checkbox / byte-checkbox-checked）
    ...
    if first_result:
        ...
        if "already" in fr:
            ...
            """, 80)
            wv_fc = json.loads(wv_r) if wv_r else wv0
            sx = wv_fc['sx'] + fr['x']
            sy = wv_fc['sy'] + fr['y']
...
def main():
    ...
    # 读取白名单（A列账号名，B列可选配额；无B列默认1）
    whitelist_quota = _read_whitelist_with_quota()
    ...
    if pending_mode:
        ...
        if whitelist_quota:
            accounts = [a for a in accounts if any(inc in a or a in inc for inc in whitelist_quota.keys())]
            log(f"账号配置.xlsx[白名单]已加载，白名单 {len(whitelist_quota)} 个，过滤后剩 {len(accounts)} 个账号")
    ...
    if pending_mode:
        ...
        def _q(a):
            for k, v in whitelist_quota.items():
                if k in a or a in k:
                    return v
            return 1
        for a in accounts:
        ...
        log(f"本次发布: {len(accounts)} 个账号，{len(docs)} 篇文档，白名单配额合计 {total_quota} 篇")
