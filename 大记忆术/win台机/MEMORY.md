...
- [4 Mac 真版位置 + 口语映射](reference_4mac真版位置.md) — 4 Mac 三大件全在 `~/Desktop/` 同款无 Mac 前缀:**「微头条」→ `微头条自动发布/`、「文章」→ `文章自动发布/`、「定时文章」/「文章定时」→ `文章定时自动发布/`**;素材库均在 `<大件>/素材/`;定时配置在 `文章定时自动发布/定时发布.xlsx` B1 改日期(2026-05-06 实测)
