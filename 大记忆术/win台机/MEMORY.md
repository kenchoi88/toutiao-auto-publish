...
- ❗ [Tailscale 网络](reference_Tailscale网络.md) — kenchoi315@gmail.com tailnet;5 机 IP:台机100.86.79.39/mini100.70.22.7/neo100.68.57.96/neo2100.96.153.17/air100.126.82.58;**ACL ssh accept + 节点 `tailscale up/set --ssh` 双门控(2026-05-16 neo 补齐),控制台缺绿标=节点漏 --ssh**;跨机命令前 grep 本表别脑补 IP
...
- ❗ [4 Mac 真版位置 + 缺哥口语映射 + 5机×3件素材路径全集](reference_4mac真版位置.md) — 缺哥固定 3 词:**「自动头条」=微头条/、「自动文章」=文章/、「定时文章」=文章定时/**;**5 机 × 3 件 = 15 条素材路径全写死(2026-05-13 拍)**,听到口语秒映射别再问
- ❗ [台机素材源池根路径](reference_台机素材源池路径.md) — 写死:头条源根 `C:\Users\kench\Desktop\台机DS创作微头条\`、文章源根 `C:\Users\kench\Desktop\台机DS创作新文章\`,每天新建`<日期-时间-N篇>`子目录;最新子目录 `ls -td <root>/*/ | head -1`,分发后台机源池删源
...
- ❗ [创新线与本钱线物理隔离 — 不卖铲子不挂AI招牌](feedback_副业方向物理隔离.md) — 头条=本钱机器+内容出海/接单=创新池,两条**平行独立永不交叉**;不卖咨询/课/工作流外包,公网匿名,作品集纯 To B(2026-05-15)
- [接单平台拳头清单 + 英文边界](reference_接单平台拳头清单.md) — 国内程客+猪八戒,国外 Upwork+Toptal;Claude cover 异步 90%,视频/时差/0 评价是真痛点;学习接单 4 步起手(2026-05-15)
- [创新线两条新项目 — 内容出海+学习接单](project_副业新项目筹划.md) — 本钱+创新双线;**接单当内容出海侦察兵,真实付费市场反推选题**;两条创新路径与头条永不交叉(2026-05-15)
- [搬家方案_阿良打包](reference_搬家方案_阿良打包.md) — Win 台机换机时阿良(air)远程 ssh 老台机打包绣虎本体 4 类(`.claude/`+`.ssh/`+桌面真版+浏览器配置),走 Tailscale scp 到 air 暂存,新机来了再分发(2026-05-15)
