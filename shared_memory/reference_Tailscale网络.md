...
| neo / 小齐+小师弟 | `neo` | 100.68.57.96 | 2026-04-27 18:25 上线(LaunchDaemon 持久化已配) |
...
- hostname 不统一(`ken-choi` / `kenchoimac-mini` / `kenmacbook-air` / `neo2`)— 是 macOS / Win 的 sharing name 默认值;以后 `tailscale up --hostname=<角色名>` 统一(neo2 已用 `--hostname=neo2`)
